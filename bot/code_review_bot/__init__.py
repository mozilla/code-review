...
class Issue(abc.ABC):
    ...
    def hash(self):
        ...
        file_lines = None
        if not self.is_autogenerated:
            content = self.revision.get_file_content(
                self.path, self.revision.local_repository
            )
            if content is not None:
                file_lines = content.splitlines()

        return self.build_hash(file_lines)

    def build_hash(self, file_lines):
        """
        Build the hash from the lines of the file affected by the issue
        (None when the file content is not available)
        See `Revision.build_issues_hashes` to build hashes for many issues at once
        """
