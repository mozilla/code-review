...
<template>
  <div>
    <h1 class="title">
      Check {{ $route.params.analyzer }} / {{ $route.params.check }}
    </h1>
    <h2 class="subtitle">On repository {{ $route.params.repository }}</h2>
    <Pagination
      :api_data="$store.state.check_issues"
      name="issues"
        ...
        <tr>
          ...
          <td>
            <Choice
              ...
              name="publishable"
              v-on:new-choice="load_issues('publishable', $event)"
            />
          </td>
          <th>Message</th>
        </tr>
    ...
    <div class="notification is-info" v-if="issues && issues.length == 0">
