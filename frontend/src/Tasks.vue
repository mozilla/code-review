      ...
      choices: {
        issues: [
          ...
          },
          {
            ...
            func: (t) =>
              t.data.issues_publishable && t.data.issues_publishable > 0,
          },
        ],
      },
    };
  ...
  computed: {
    tasks() {
      ...
      if (this.filters.repository !== null) {
        tasks = _.filter(
          tasks,
          (t) => t.data.repository === this.filters.repository
        );
      }
        ...
        .map((state) => {
          ...
          return {
            ...
            name: state.startsWith("error.")
              ? "error: " + state.substring(6)
              : state,
            nb,
            percent:
              currentTasks && currentTasks.length > 0
                ? Math.round((nb * 100) / currentTasks.length)
                : 0,
          };
        })
        .sort((x, y) => {
...
<template>
  <section>
    <div class="buttons has-addons is-pulled-right">
      <button
        v-on:click="load_tasks('production')"
        class="button is-small"
        :class="{ 'is-primary': channel == 'production' }"
      >
        Production tasks
      </button>
      <button
        v-on:click="load_tasks('testing')"
        class="button is-small"
        :class="{ 'is-primary': channel == 'testing' }"
      >
        Testing tasks
      </button>
    </div>
        ...
        <div class="column is-one-third">
          <progress
            ...
            :class="{
              ...
              'is-info': state.key != 'done' && !state.key.startsWith('error'),
            }"
            :value="state.percent"
            max="100"
          >
        ...
        <tr>
          ...
          <td>
            <Choice
              ...
              name="repo"
              v-on:new-choice="filters.repository = $event"
            />
          </td>
          <td>
            <Choice
              :choices="states"
              name="state"
              v-on:new-choice="filters.state = $event"
            />
          </td>
          <td>
            <Choice
              ...
              name="issue"
              v-on:new-choice="filters.issues = $event"
            />
          </td>
          <td>Indexed</td>
          <td>Actions</td>
        ...
        <tr v-for="task in tasks">
          ...
          <td>
            ...
            <span class="tag is-dark" v-else>{{
              task.data.repository || "Unknown"
            }}</span>
          </td>

          <td>
            <span class="tag is-light" v-if="task.data.state == 'started'"
              >Started</span
            >
            <span class="tag is-info" v-else-if="task.data.state == 'cloned'"
              >Cloned</span
            >
            <span class="tag is-info" v-else-if="task.data.state == 'analyzing'"
              >Analyzing</span
            >
            <span
              class="tag is-primary"
              v-else-if="task.data.state == 'analyzed'"
              >Analyzed</span
            >
            ...
            <span
              class="tag is-danger"
              v-else-if="task.data.state == 'error'"
              :title="task.data.error_message"
            >
              Error: {{ task.data.error_code || "unknown" }}
            </span>
            <span class="tag is-success" v-else-if="task.data.state == 'done'"
              >Done</span
            >
          ...
          <td :class="{ 'has-text-success': task.data.issues_publishable > 0 }">
            <span v-if="task.data.issues_publishable > 0">{{
              task.data.issues_publishable
            }}</span>
            <span v-else-if="task.data.issues_publishable == 0">{{
              task.data.issues_publishable
            }}</span>
            <span v-else>-</span>
            / {{ task.data.issues }}
          ...
          <td>
            <a class="button is-link" :href="task.data.url" target="_blank"
              >Phabricator</a
            >
            <a
              v-if="task.data.bugzilla_id"
            ...
            >
            <router-link
              v-if="task.data.issues > 0"
              :to="{ name: 'diff', params: { diffId: task.data.diff_id } }"
              class="button is-primary"
              >Issues</router-link
            >
          </td>
